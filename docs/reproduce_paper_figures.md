...
| Analytical no-go volume | `tools/run_analytical_grasp_volume.py` | Coarse YCB-like object bounding boxes, analytical grasp poses, inflated-AABB linear approach proxy | `artifacts/analytical_grasp_volume/summary.csv`, `volume.csv`, `*_viability_slices.png`, `*_no_go_volume.png` | No | Simple analytical grasping has a characterized no-go volume where a linear Cartesian move cannot reach the grasp pose without object-aware path planning. |
