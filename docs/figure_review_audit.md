...
| `artifacts/analytical_grasp_volume/*_viability_slices.png` | Where do simple analytical grasps fail before needing object-aware path planning? | For coarse YCB-like object proxies, each voxel shows the fraction of sampled hand yaw poses that can linearly reach the analytical grasp pose without crossing an inflated object proxy. | It is not full MuJoCo collision, arm IK, learning comparison, or a hardware result. | New core characterization figure; refine before paper submission. |
