...
Viewer rotation convention: MuJoCo applies $R_x(r_x) R_y(r_y) R_z(r_z)$ (axes in parent
frame). For the default top-down approach ($\mathbf{R}_\text{plane} = I$), FK uses
$R = R_y(\theta) R_x(\pi)$. The XYZ Euler extraction gives:

$$r_x = \pi, \quad r_y = -\theta, \quad r_z = 0$$

Identity: $R_x(\pi) R_y(-\theta) = R_y(\theta) R_x(\pi)$.

For non-zero plane orientations the full $\mathbf{R}_\text{full}$ is used (see
[Arbitrary Plane Orientation](#arbitrary-plane-orientation) below).
...
The default world-frame conversion approaches the grasp plane from directly above
(world $-Z$), tilted by $\theta$ only in the spread axis. To grasp objects on inclined
surfaces, from the side, or at an arbitrary angle, a user-specified *plane rotation* is
applied on top of the auto-tilt.

### Extended Rotation

Let $(\phi_x, \phi_y, \phi_z) \in [-\pi, \pi]^3$ be user-controlled plane orientation
angles (radians; sliders in the UI provide degrees). Define:

$$\mathbf{R}_\text{plane}(\phi_x, \phi_y, \phi_z) = R_x(\phi_x)\, R_y(\phi_y)\, R_z(\phi_z)$$

The full base-to-world rotation becomes:

$$\boxed{\mathbf{R}_\text{full} = \mathbf{R}_\text{plane}(\phi_x,\phi_y,\phi_z)\, R_y(\theta)\, R_x(\pi)}$$

World-frame conversion uses $\mathbf{R}_\text{full}$ in place of $R(\theta)$:

$$\mathbf{m}_w = \mathbf{R}_\text{full}\,\mathbf{m}_b, \qquad
  \mathbf{b}_w = \begin{pmatrix}-m_{w,x}\\-m_{w,y}\\g_z-m_{w,z}\end{pmatrix}, \qquad
  \mathbf{p}_w(f) = \mathbf{R}_\text{full}\,\mathbf{p}_b(f) + \mathbf{b}_w$$

The grasp midpoint continues to land at $(0,0,g_z)$ regardless of plane orientation.

### XYZ Euler Extraction (viewer sync)

Given $\mathbf{R}_\text{full}$, the MuJoCo viewer requires intrinsic XYZ Euler angles:

$$r_y = \arcsin\!\bigl([\mathbf{R}_\text{full}]_{0,2}\bigr)$$

$$r_x = \operatorname{atan2}\!\bigl(-[\mathbf{R}_\text{full}]_{1,2},\;[\mathbf{R}_\text{full}]_{2,2}\bigr)$$

$$r_z = \operatorname{atan2}\!\bigl(-[\mathbf{R}_\text{full}]_{0,1},\;[\mathbf{R}_\text{full}]_{0,0}\bigr)$$

(Standard gimbal-lock fallback when $\cos r_y \approx 0$.) This replaces the previous
closed-form $r_x=\pi$, $r_y=-\theta$, $r_z=0$, which is recovered exactly when
$\mathbf{R}_\text{plane} = I$.

Implemented in `GraspVizCore._mat_to_xyz_euler()`.
...
|---|---|---|
| `ctrl_max[pinky]`, `ctrl_max[ring]` | 1.57 rad | XML actuator ctrlrange |
| `ctrl_max[middle]`, `ctrl_max[index]` | 1.50 rad | XML actuator ctrlrange |
| `ctrl_min[thumb_proximal]`, `ctrl_max[thumb_proximal]` | 0.1, 0.57 rad | XML actuator ctrlrange |
| `ctrl_max[thumb_yaw]` | 1.308 rad | XML actuator ctrlrange |
| `ctrl[thumb_yaw]` (2-finger line) | 1.16 rad | empirical constant `_THUMB_YAW_LINE` |
| Thumb preload (offset) | 0.15 rad (≈ 8.6°) | polycoef `a0` (thumb only) |
| Finger Z-shift (open → closed) | ≈ 75 mm | FK sweep |
| Cylinder yaw transition diameter | 71.6 mm | tip-clearance sweep |
| FK samples (non-thumb) | 200 per finger | `N_SAMPLES_1D` |
| FK samples (thumb) | 50 × 50 | `N_SAMPLES_2D` |

---

## Mink Differential IK Grasp Planner

`MinkGraspPlanner` (`mink_grasp_planner.py`) is an alternative grasp synthesis path
based on iterative differential IK using the `mink` library (QP-based IK for MuJoCo
models). It serves as a research baseline for comparing against the analytical
`ClosureGeometry` solver. Comparison plots are generated by `compare_grasp_methods.py`.

### QP Formulation

At each IK iteration the following constrained least-squares problem is solved for joint
velocity $\dot{\mathbf{q}} \in \mathbb{R}^{12}$:

$$\min_{\dot{\mathbf{q}}} \;\;
    \sum_{f \in \mathcal{A}} \bigl\|J_f \dot{\mathbf{q}} - v_f^*\bigr\|_2^2
    + w_c \bigl\|C\dot{\mathbf{q}}\bigr\|_2^2
    + w_d \bigl\|\dot{\mathbf{q}}\bigr\|_2^2
  \quad\text{subject to}\quad
  \dot{\mathbf{q}}_{\min} \le \dot{\mathbf{q}} \le \dot{\mathbf{q}}_{\max}$$

where:
- $\mathcal{A}$ — active fingers for the current grasp mode
- $J_f \in \mathbb{R}^{3\times12}$ — translational Jacobian of finger $f$'s tip site
- $v_f^* = (p_f^* - p_f^\text{cur}) / \Delta t$ — target tip velocity; $p_f^*$ is
  from `ClosureGeometry`'s FK output; $\Delta t = 0.005\,\text{s}$
- $C\dot{\mathbf{q}} = 0$ — six polycoef equality constraints (Table above) via mink's
  `EqualityConstraintTask` at cost $w_c = 50$
- $w_d = 10^{-2}$ — isotropic damping (`DampingTask`)
- Joint limits enforced via `ConfigurationLimit` and `VelocityLimit` ($\pi$ rad/s max)

Solved with `daqp` (active-set dual method). Configuration integrated as
$\mathbf{q} \leftarrow \mathbf{q} + \dot{\mathbf{q}}\,\Delta t$ after each step.

### Task Weights per Grasp Mode

**2-finger line**: Thumb yaw frozen at $c_\text{yaw}^\text{line} = 1.16$ rad by a
heavy yaw-damping term ($100\times$ standard weight). Only thumb-tip and index-tip
`FrameTask`s active.

**3/4/5-finger plane**: Thumb yaw fixed at $c_\text{yaw}^\max = 1.308$ rad. One
`FrameTask` per active fingertip, equal weights. Coplanarity emerges implicitly because
all non-thumb tips share the same target $Z$ derived from the analytical solver.

**Cylinder (power grasp)**: Same as plane mode; all five fingers active.

### Convergence Criterion

Terminates when total per-finger tip error satisfies:

$$\sum_{f \in \mathcal{A}} \|p_f - p_f^*\| < \varepsilon \cdot |\mathcal{A}|
  \qquad \varepsilon = 5\,\text{mm (default)}$$

or when the iteration limit is reached (150 default; 500 in `compare_grasp_methods`).
Non-converged results are flagged in comparison plots with red markers.

### Coplanarity Metric

Both planners are evaluated on the mean absolute world-frame $Z$ deviation of non-thumb
fingertips from their mean world-$Z$:

$$\varepsilon_\text{coplan} = \frac{1}{N}\sum_{i=1}^{N}
    \bigl|w_{z,i} - \bar{w}_z\bigr|, \qquad
    w_{z,i} = \bigl(\mathbf{R}(\theta)\,\mathbf{p}_i + \mathbf{b}_w\bigr)_z$$

The analytical solver enforces equal *base-frame* $Z$ (not world-frame $Z$), so its
world-frame spread is:

$$\varepsilon_\text{coplan}^\text{analytical} \approx
  \tfrac{1}{2}\,\text{range}\!\bigl(\sin\theta\,(p_{x,i} - \bar{p}_x)\bigr)
  \approx 2\text{–}5\,\text{mm}$$

This is a fundamental hardware constraint (different finger lengths) and is accepted.

### UR5 Robot Viewer Integration

When the **Robot: Ours** or **Robot: Mink** viewer opens, a dedicated subprocess loads
`ur5_inspire.xml` and runs:

1. **Arm IK** (always mink): a `FrameTask` targets the `eeff` site at the desired SE(3)
   pose from the current grasp base position and tilt angle. Five iterations of
   $\Delta t = 0.05\,\text{s}$ mink IK per visual frame drive the six UR5 arm joints.

2. **Finger actuators**: set directly from the selected planner's
   $\mathbf{c} \in \mathbb{R}^6$ (analytical or mink), bypassing the arm IK.

The two viewers (**Ours** vs. **Mink**) differ only in the finger control values; arm
motion is identical for both.
